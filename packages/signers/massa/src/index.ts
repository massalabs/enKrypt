...
export class MassaSigner implements SignerInterface {
  ...
  async sign(msgHash: string, keyPair: KeyPair): Promise<string> {
    ...
    const bugToSign = hexToBuffer(msgHash);
    console.log('🟡 [MASSA_SIGNER] bugToSign:', bugToSign);
    console.log('🟡 [MASSA_SIGNER] bugToSign length:', bugToSign.length);
    const sig = await privateKey.sign(bugToSign);
