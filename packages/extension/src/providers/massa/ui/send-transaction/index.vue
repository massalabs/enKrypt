...
onMounted(async () => {
  ...
  try {
    ...
    console.log('🟡 [UI] Got Request from window promise:', Request.value);
...
const sendAction = async () => {
  ...
  try {
    ...
    console.log('🟡 [UI] Transaction params:', transactionParams);

    // Debug windowPromise
    console.log('🟡 [UI] windowPromise:', windowPromise);
    console.log('🟡 [UI] windowPromise type:', typeof windowPromise);
    
    // Get the Resolve function from the window promise
    console.log('🟡 [UI] Awaiting windowPromise...');
    const { Resolve } = await windowPromise;
    console.log('🟡 [UI] Got Resolve function:', Resolve);
    console.log('🟡 [UI] Resolve type:', typeof Resolve);
    console.log('🟡 [UI] Resolve.value:', Resolve.value);
    ...
    console.log('🟡 [UI] Sending response to background:', response);
    
    // Send the transaction parameters back to the background script
    console.log('🟡 [UI] Calling Resolve.value(response)...');
    Resolve.value(response);

    console.log('✅ [UI] Transaction sent successfully');
    close();
  } catch (error) {
    console.error('❌ [UI] Error sending transaction:', error);
